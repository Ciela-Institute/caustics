...
package [@Herculens]; among others [GRAVLENS @Keeton2011; LENSTOOL
@Kneib2011; SLITRONOMY @Galan2021; paltax @Wagner2024]. There are also several "in
house" codes developed for specialized analysis which are then not publicly released
[e.g. @Suyu2010]. `Caustics` development has been primarily focused on three
aspects: processing speed, user experience, and flexibility. The code is
...
experience may effectively engage with the software. Flexibility is achieved by
...
into a single simulator which users may access and use simply as a function of the
relevant lensing and light source parameters. Since `caustics` is written in `PyTorch`,

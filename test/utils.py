...
from caustic.display_utils import display
...
def alpha_test_helper(lens, lens_ls, z_s, x, kwargs_ls, atol, rtol):
    ...
    display(alpha_x, title=r"$\alpha_x$ caustic")
    display(alpha_x_ls, title=r"$\alpha_x$ lenstronomy")
    display(alpha_x - alpha_x_ls, title="diff")
